...
if DB_SSL_CA:
    # Aiven requires sslrootcert with psycopg2 for secure connections
    ca_path = os.path.join(os.path.dirname(__file__), "..", DB_SSL_CA)
    # Using sslrootcert requires passing sslmode=require
    DATABASE_URL = f"postgresql+psycopg2://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/{DB_NAME}?sslmode=require&sslrootcert={ca_path}"
else:
